...
def inject_fake_gps_origin(mav: MAVLinkController):
    """Sends both EKF origin and Home position to bypass the 'requires position' safety check indoors."""
    log.info("Sending fake GPS global origin and home position to initialize EKF...")
    try:
        sys_id = int(mav.connection.target_system)

        # Coordinates: Static reference point frame
        lat = 521234560  # Deg * 1E7
        lon = 211234560  # Deg * 1E7
        alt = 150000  # mm (150 meters)

        # 1. Send EKF Global Origin
        # ArduPilot expects: target_system, latitude, longitude, altitude
        mav.connection.mav.set_gps_global_origin_send(sys_id, lat, lon, alt)
        log.info("  -> EKF Global Origin injected.")
        time.sleep(0.5)

        # 2. Send Home Position (Required alongside Origin for GUIDED mode activation)
        # Parameters: target_system, latitude, longitude, altitude, x, y, z, q(4), approach_x, approach_y, approach_z
        mav.connection.mav.set_home_position_send(
            sys_id,
            lat,
            lon,
            alt,
            0,
            0,
            0,  # x, y, z relative offsets
            [1, 0, 0, 0],  # Quaternion orientation
            0,
            0,
            0,  # Approach vector
        )
        log.info("  -> Home Position injected.")

        # 3. Give ArduPilot EKF a moment to cycle and clear its error state
        log.info("Waiting 3 seconds for EKF to stabilize on virtual map...")
        time.sleep(3.0)
        log.info("  -> Fake GPS injection complete! Check QGC map.")
